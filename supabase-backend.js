(function () {
  ...
  async function verifySlip(file, studentKey) {
    ...
    const response = await fetch(`${config.url}/functions/v1/verify-slip`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || code === "NOT_FOUND") {
        throw new Error("VERIFY_SLIP_NOT_DEPLOYED");
      }
      const providerDetail = [
        data?.providerCode,
        data?.providerMessage || data?.message
      ].filter(Boolean).join(":");
      throw new Error([
        code || `VERIFY_SLIP_HTTP_${response.status}`,
        providerDetail
      ].filter(Boolean).join("|"));
    }
